...
import { ALL_ROLES, parseRoles } from "../helpers.js";

const r = Router();
r.use(authRequired, requireRole("admin"));

const out = (u) => { const roles = parseRoles(u.role); return { id: u.id, name: u.name, login: u.login, role: roles[0] || "", roles, hotelIds: JSON.parse(u.hotel_ids || "[]"), mustChange: !!Number(u.must_change) }; };
// aceita roles[] (novo) ou role (compat) e normaliza para lista válida e sem duplicatas
const readRoles = (body) => {
  const raw = Array.isArray(body.roles) ? body.roles : (body.role != null ? [body.role] : []);
...
r.post("/", async (req, res, next) => {
  try {
    const { name, login, password, hotelIds } = req.body || {};
    const roles = readRoles(req.body || {});
    if (!name || !login || !password || !roles.length) return res.status(400).json({ error: "Dados incompletos — informe ao menos um perfil." });
    if (roles.some((r) => !ALL_ROLES.includes(r))) return res.status(400).json({ error: "Perfil inválido." });
    ...
    const id = newId();
    const hs = roles.includes("admin") ? "[]" : JSON.stringify(Array.isArray(hotelIds) ? hotelIds : []);
    await db.run("INSERT INTO users (id,name,login,pass_hash,role,hotel_ids,must_change,created_at) VALUES (?,?,?,?,?,?,1,?)",
      [id, name.trim(), String(login).trim(), hashPassword(password), roles.join(","), hs, nowISO()]);
...
r.put("/:id", async (req, res, next) => {
  try {
    const { name, login, password, hotelIds, resetPassword } = req.body || {};
    const roles = readRoles(req.body || {});
    const row = await db.get("SELECT * FROM users WHERE id = ?", [req.params.id]);
    if (!row) return res.status(404).json({ error: "Usuário não encontrado." });
    if (!roles.length) return res.status(400).json({ error: "Informe ao menos um perfil." });
    if (roles.some((r) => !ALL_ROLES.includes(r))) return res.status(400).json({ error: "Perfil inválido." });
    if (login) {
    ...
    await db.run("UPDATE users SET name=?, login=?, role=?, hotel_ids=? WHERE id=?",
      [(name || row.name).trim(), (login || row.login).trim(), roles.join(","), hs, row.id]);
